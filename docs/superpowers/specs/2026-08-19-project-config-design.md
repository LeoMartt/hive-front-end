...
`addTeamMember` funciona igual ao `createProject` de hoje: `setProjects` mapeando o projeto pelo id e fazendo `[...project.team, member]` — um append direto, sem validação própria (mesma simplicidade de `createProject`). A checagem de duplicidade continua vivendo na camada de UI, como já é hoje em `NewProjectModal.handleAddUser` — só que `InviteUserModal` compara por `name` + `role` (não por `id`, porque membros vindos do seed de `useProjects.ts` não têm `id`; comparar por `id` deixaria passar um convite duplicado do Graph para alguém que já está no time via seed). `replaceTeamMemberRoles` remove todas as entradas `TeamMember` daquele `name` no `team` do projeto e recria uma por papel em `roles` (reaproveitando `initials`/`email` da entrada existente, ou derivando `initials` via `getInitials(memberName)` se não houver nenhuma entrada prévia) — é a mesma operação de "readicionar com outro papel" que `NewProjectModal` já faz, só que em lote/via edição.
...
**`InviteUserModal`**: `show`/`onHide`/`currentTeam: TeamMember[]`/`onInvite: (member: TeamMember) => void`. Usa `useGraphUserSearch()` para a busca + um `<select>` de papel (mesmas 3 opções de `NewProjectModal`). No "Convidar", valida duplicidade contra `currentTeam` (por `name`+`role`, mesma lógica de `NewProjectModal.handleAddUser` só que comparando por nome) — mostra erro inline se já existir, senão chama `onInvite(member)` e fecha. `ConfigUsersTable` passa `project.team` como `currentTeam` e liga `onInvite` a `addTeamMember(projectId, member)`.
