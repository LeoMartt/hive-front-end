...
- `ISSUE_STATUS_BADGE_CLASS: Record<IssueStatus, string>` — reaproveita as classes de cor já existentes em `_activities.scss`: `aberta → "activity-badge-bloqueado"` (vermelho), `em_analise → "activity-badge-liberado"` (amarelo — `activity-badge-execucao` é azul neste projeto, não amarelo como o `st-execucao` do mockup; `activity-badge-liberado` é a classe que já é amarela), `solucao_proposta → "activity-badge-liberado"` (amarelo — mesmo tom que Em análise, replicando o mockup), `concluida → "activity-badge-concluido"` (verde). Nenhum CSS novo para isso.
