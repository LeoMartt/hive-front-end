...
**Nota pós-implementação:** a primeira versão deste hook usava `location.key !== "default"`. Uma revisão de qualidade encontrou que `location.key` também muda em navegações "replace" (não só "push"), o que o tornaria enganoso caso alguma tela futura chegasse a um desses detalhes via redirect — por isso a versão final usa `history.state.idx`, que só avança em "push". Ver commit `41939df`.
